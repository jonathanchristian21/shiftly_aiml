...
<div class="card p-6 mb-6" style="background: linear-gradient(135deg, #EFF5FF 0%, #DBEAFE 100%); border-color: #BFDBFE;">
    <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
            ...
            <p class="text-body mb-4">Clustering is not just grouping employees. <strong>This is a pre-optimization phase</strong> that makes GA's population initialization much smarter and not totally random.</p>
            
            <div class="bg-white rounded-lg p-4 mb-4">
                <div class="text-tiny mb-3">6 INPUT FEATURES (NORMALIZED 0-1):</div>
            ...
            <div class="border-l-4 border-sky pl-4">
                ...
                <p class="text-caption leading-relaxed">
                    When GA creates <strong>initial population</strong>, each shift per department is guaranteed to be filled with <strong>at least 1 employee from Senior Cluster</strong>. 
...
<div class="card p-6 mb-8">
    ...
    <p class="text-body text-ink-mute mb-5">Run K-Means to segment employees. Results will be used by GA for better quality initial population.</p>
    ...
    <form method="POST" action="{{ route('manager.cluster.start') }}" onsubmit="return confirm('Start clustering? This will update cluster labels for all active employees.')">
        ...
        <div class="flex items-end gap-4">
            <div>
                <label class="text-tiny mb-2 block">Number of Clusters</label>
                <select name="n_clusters" class="w-full mono">
                    @for($i = 2; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ $i === 3 ? 'selected' : '' }}>K = {{ $i }}</option>
                    @endfor
                </select>
                <p class="text-caption mt-1">Recommended: 3-5 clusters</p>
            </div>
        ...
        <div class="border border-gray-200 rounded-lg p-5 card-hover">
            ...
            <div class="space-y-2.5">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-body font-semibold mono">{{ $analysis['pg_percentage'] }}%</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-caption flex items-center gap-2">
                        <svg class="w-3.5 h-3.5 text-ink-mute" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span class="text-body font-semibold mono">{{ $analysis['senior_count'] }}</span>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="text-caption bg-gray-50 rounded-lg p-3 mono leading-relaxed">
                    @php
                        $avgJobLevel = $analysis['avg_job_level'];
                        $pgPct = $analysis['pg_percentage'];
                        if ($avgJobLevel >= 4 && $pgPct >= 70) {
                            echo '<span class="text-emerald-600">● SENIOR EXPERTS</span><br>';
                            echo 'High experience + PG dominant';
                        } elseif ($avgJobLevel >= 3 && $analysis['avg_rating'] >= 4) {
                            echo '<span class="text-sky">● MID-LEVEL HIGH PERFORMERS</span><br>';
                            echo 'Flexible, high rating staff';
                        } elseif ($avgJobLevel < 2.5) {
                            echo '<span class="text-amber-600">● JUNIOR STAFF</span><br>';
                            echo 'Entry-level employees';
                        } else {
                            echo '<span class="text-purple-600">● MIXED PROFILE</span><br>';
                            echo 'Diverse characteristics';
                        }
                    @endphp
                </div>
            </div>
